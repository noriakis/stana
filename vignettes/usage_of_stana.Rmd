...
```{r genes, fig.width=8}
plotGenes(stana, "100003", geneID=stana@genes[[1]] |> row.names() |> head(3))
```

### Plotting `circlize` plot

In this example, mean minor allele frequency of SNV on genes in the genome `UHGG000008` per group is plotted per track.

```{r circ, warning=FALSE, message=FALSE}
library(circlize)
plotCirclize(stana, "100003",
            genomeId="UHGG000008",
            thresh_snp_gene = 20,
            MAF=TRUE)
```
